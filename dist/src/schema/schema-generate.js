...
import { hasSchemaInput, parseJsonFlag } from "./schema-utils.js";
export function generateSchemaJsonLd(input) {
    const generated = [];
...
export function runSchemaGenerate(input) {
...
export function parseSchemaInputFromText(rawInput) {
    const input = { mode: rawInput.includes("schema-generate") ? "generate" : "audit" };
    ...
    for (const match of args.matchAll(flagPattern)) {
        ...
        if (key === "html")
            input.html = value;
        if (key === "jsonLd" || key === "json-ld")
            input.jsonLd = parseJsonFlag(value, []);
        if (key === "page")
            input.page = parseJsonFlag(value, undefined);
        if (key === "organization")
            input.organization = parseJsonFlag(value, undefined);
        if (key === "author")
            input.author = parseJsonFlag(value, undefined);
        if (key === "product")
            input.product = parseJsonFlag(value, undefined);
        if (key === "service")
            input.service = parseJsonFlag(value, undefined);
        if (key === "softwareApplication" || key === "software-application")
            input.softwareApplication = parseJsonFlag(value, undefined);
        if (key === "localBusiness" || key === "local-business")
            input.localBusiness = parseJsonFlag(value, undefined);
        if (key === "video")
            input.video = parseJsonFlag(value, undefined);
        if (key === "jobPosting" || key === "job-posting")
            input.jobPosting = parseJsonFlag(value, undefined);
        if (key === "mode" && ["audit", "generate", "validate", "planning"].includes(value))
            input.mode = value;
    }
    return input;
}
