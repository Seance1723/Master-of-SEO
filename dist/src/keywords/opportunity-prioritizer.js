export function recommendPageType(intent, business) {
    if (intent === "informational")
        return "blog";
    if (intent === "commercial" || intent === "comparison")
        return "comparison";
    if (intent === "transactional")
        return business?.websiteType === "ecommerce" ? "product" : "service";
    if (intent === "pricing")
        return "pricing";
    if (intent === "local")
        return "local";
    ...
    if (intent === "navigational")
        return "homepage";
    if (intent === "product_led")
        return "product";
    return "unknown";
}
export function scoreBusinessValue(keyword, intent, business) {
    ...
    if (["transactional", "pricing", "local", "product_led"].includes(intent))
        return "high";
    if (intent === "commercial" && matchesBusiness)
        return "high";
    if (["comparison", "support"].includes(intent) || (intent === "informational" && matchesBusiness))
        return "medium";
    if (intent === "informational")
        return "low";
    return "unknown";
}
export function findOpportunities(clusters, metrics, competitorKeywords) {
    const opportunities = [];
...
//# sourceMappingURL=opportunity-prioritizer.js.map