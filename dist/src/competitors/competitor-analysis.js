const commercialIntents = new Set(["commercial", "transactional", "pricing", "product_led", "local", "comparison"]);
export function runCompetitorAnalysis(input) {
    if (!hasUsableInput(input))
        return needsInput(isUrlOnly(input));
...
export function parseCompetitorAnalysisInputFromText(rawInput) {
    ...
    if (urls.length)
        input.competitors = urls.map((url) => ({ domain: domainFromUrl(url) }));
    for (const match of args.matchAll(/--([a-zA-Z][\w-]*)(?:\s+(?:"([^"]*)"|'([^']*)'|(\S+)))?/gu)) {
        const key = match[1];
        const value = match[2] ?? match[3] ?? match[4] ?? "";
        if (key === "business")
            input.business = parseJsonFlag(value, undefined);
        if (key === "ownSite" || key === "own-site")
            input.ownSite = parseJsonFlag(value, undefined);
        if (key === "competitors")
            input.competitors = parseJsonFlag(value, []);
        if (key === "serpData" || key === "serp-data")
            input.serpData = parseJsonFlag(value, []);
    }
    return input;
}
export function detectCompetitorTypes(input) {
    ...
    return (input.competitors ?? []).map((competitor) => {
        const evidence = [];
        if (competitor.type)
            return { competitor: competitor.name ?? competitor.domain ?? "competitor", type: competitor.type, evidence: ["Provided competitor type"] };
        const domain = normalize(competitor.domain);
        const pages = competitor.pages ?? [];
        if (domain && serpDomains.has(domain))
            evidence.push("Appears in provided SERP data");
        if (pages.length && pages.every((page) => ["blog", "article"].includes(page.pageType ?? "unknown")))
            return { competitor: competitor.name ?? competitor.domain ?? "competitor", type: "content", evidence: ["Mostly blog/article pages"] };
        if (pages.some((page) => page.pageType === "local") || (competitor.keywords ?? []).some((keyword) => keyword.intent === "local"))
            return { competitor: competitor.name ?? competitor.domain ?? "competitor", type: "local", evidence: ["Local page or local intent evidence"] };
        if (pages.some((page) => ["product", "category"].includes(page.pageType ?? "")))
            return { competitor: competitor.name ?? competitor.domain ?? "competitor", type: "product", evidence: ["Product/category page overlap"] };
        if (/marketplace|directory|listing|compare/iu.test(`${competitor.domain ?? ""} ${competitor.name ?? ""}`))
            return { competitor: competitor.name ?? competitor.domain ?? "competitor", type: "marketplace", evidence: ["Marketplace/listing pattern"] };
        if (evidence.length)
            return { competitor: competitor.name ?? competitor.domain ?? "competitor", type: "search", evidence };
        if (input.business?.industry && pages.length)
            return { competitor: competitor.name ?? competitor.domain ?? "competitor", type: "direct", evidence: ["Business industry and competitor pages provided"] };
        return { competitor: competitor.name ?? competitor.domain ?? "competitor", type: "unknown", evidence: ["Insufficient role evidence"] };
    });
}
function analyzeSerp(input, ownTargets) {
    return (input.serpData ?? []).map((serp) => {
        const counts = new Map();
        for (const result of serp.topResults ?? [])
            counts.set(result.pageType ?? "unknown", (counts.get(result.pageType ?? "unknown") ?? 0) + 1);
...
function analyzeKeywordGaps(input, ownKeywords) {
    const gaps = [];
    for (const competitor of input.competitors ?? []) {
        const keywords = [...(competitor.keywords ?? []), ...(competitor.pages ?? []).flatMap((page) => (page.rankingKeywords ?? []).map((keyword) => ({ keyword, url: page.url, intent: inferIntent(keyword) })))];
        for (const item of keywords) {
            const key = normalize(item.keyword);
            if (!key || ownKeywords.has(key))
                continue;
...
function analyzeContentGaps(input, ownTargets) {
    ...
    for (const competitor of input.competitors ?? []) {
        for (const page of competitor.pages ?? []) {
            ...
            if (target && !ownTargets.has(target))
                gaps.push({ type: "target_keyword_gap", keyword: page.targetKeyword, competitorPage: page.url, pageType: page.pageType, priority: commercialPageType(page.pageType) ? "P1" : "P2", recommendation: "Create a differentiated page or improve an existing relevant page." });
            if (page.pageType && !ownPageTypes.has(page.pageType))
                gaps.push({ type: "missing_page_type", pageType: page.pageType, competitorPage: page.url, priority: commercialPageType(page.pageType) ? "P1" : "P3", recommendation: "Evaluate whether this page type supports your business goal before creating it." });
            for (const section of page.contentSections ?? []) {
                if (/faq|case stud|proof|comparison|pricing|demo|support|documentation/iu.test(section))
                    gaps.push({ type: "section_gap", section, competitorPage: page.url, priority: /pricing|demo|case stud|proof/iu.test(section) ? "P2" : "P3", recommendation: "Add original, useful coverage where it improves user decisions." });
            }
        }
    }
    return uniqueBy(gaps, (gap) => `${gap.type}:${gap.keyword ?? gap.pageType ?? gap.section}:${gap.competitorPage}`);
}
function analyzeBacklinkGaps(input) {
    const gaps = [];
    for (const competitor of input.competitors ?? []) {
...
function analyzePageStructure(input) {
...
function collectOwnKeywords(input) {
    return new Set((input.ownSite?.pages ?? []).flatMap((page) => [page.targetKeyword, ...(page.rankingKeywords ?? [])]).map(normalize).filter(Boolean));
}
function gapPriority(intent, rank) {
    if (commercialIntents.has(intent) && (!rank || rank <= 10))
        return "P1";
    if (commercialIntents.has(intent) || (rank !== undefined && rank <= 10))
        return "P2";
    return "P3";
}
function inferIntent(keyword) {
    const value = keyword.toLowerCase();
    if (/\b(price|pricing|cost)\b/u.test(value))
        return "pricing";
    if (/\b(vs|versus|alternative|compare|best)\b/u.test(value))
        return "comparison";
    if (/\b(buy|hire|book|demo|trial|service|software)\b/u.test(value))
        return "transactional";
    if (/\b(near me|city|local)\b/u.test(value))
        return "local";
    if (/\b(how|what|why|guide|tips)\b/u.test(value))
        return "informational";
    return "unknown";
}
function commercialPageType(type) {
    return ["service", "product", "category", "landing", "pricing", "comparison", "local"].includes(type ?? "");
}
function hasUsableInput(input) {
    if (input.business || input.ownSite || input.serpData?.length)
        return true;
...
function getMissingInputs(input) {
    return ["business", "ownSite", "competitors", "serpData"].filter((key) => !input[key]);
}
function needsInput(urlOnly = false) {
...
function domainFromUrl(url) {
    try {
        ...
        return url.replace(/^https?:\/\//iu, "").split("/")[0]?.replace(/^www\./u, "").toLowerCase() ?? url;
    }
}
