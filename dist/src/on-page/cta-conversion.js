export const ctaConversionRules = [{
...
export function checkCtaConversion(input) {
    const ctas = input.ctas ?? [];
    const issues = [];
    if (isCommercial(input) && ctas.length === 0)
        issues.push(issue("cta-missing-commercial-page", "Commercial page has no CTA", input.pageType === "landing" || input.pageType === "pricing" ? "P1" : "P2", "No CTA was provided or extracted.", "Add a clear CTA that matches page intent."));
    for (const cta of ctas) {
        if (/^(submit|click|go|more)$/iu.test(cta.text.trim()))
            issues.push(issue("cta-vague-text", "CTA text is vague", "P3", cta.text, "Use specific CTA copy such as Book a demo or Get pricing."));
    }
    if (isCommercial(input) && ctas.length > 0 && !ctas.some((cta) => cta.position === "above_fold"))
        issues.push(issue("cta-no-above-fold", "No above-fold CTA", "P2", "No CTA is marked above_fold.", "Place the primary CTA above the fold when relevant."));
...
//# sourceMappingURL=cta-conversion.js.map