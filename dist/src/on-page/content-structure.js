export const contentStructureRules = [{
...
export function checkContentStructure(input) {
    ...
    if (isImportant(input) && input.bodyText !== undefined && words < 150)
        issues.push(issue("content-thin-important-page", "Important page has thin body text", input.pageType === "product" || input.pageType === "service" ? "P1" : "P2", `${words} words found.`, "Add useful, intent-matching content and proof."));
    if (input.bodyText && /^(welcome to|in today's digital world|in the ever-evolving|are you looking for)/iu.test(input.bodyText.trim()))
        issues.push(issue("content-vague-intro", "Vague intro detected", "P3", input.bodyText.slice(0, 90), "Start with specific value or the direct answer."));
    return { issues, passedChecks: input.bodyText && !issues.length ? ["Content structure checks passed."] : [] };
}
function isImportant(input) {
