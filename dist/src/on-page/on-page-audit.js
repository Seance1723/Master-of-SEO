import { checkAboveTheFold } from "./above-the-fold.js";
import { checkContentStructure } from "./content-structure.js";
import { checkCtaConversion } from "./cta-conversion.js";
import { checkHeadings } from "./headings.js";
import { checkImageAlt } from "./image-alt.js";
import { checkMetaDescription } from "./meta-description.js";
import { checkOnPageLinks } from "./on-page-links.js";
import { buildOnPageRecommendations } from "./on-page-recommendations.js";
import { checkTitleTag } from "./title-tag.js";
export function runOnPageAudit(input) {
...
export function parseOnPageAuditInputFromText(rawInput) {
    ...
    for (const match of args.matchAll(flagPattern)) {
        ...
        if (key === "title")
            input.title = value;
        if (key === "metaDescription" || key === "meta-description")
            input.metaDescription = value;
        if (key === "h1")
            input.h1 = value;
        if (key === "bodyText" || key === "body-text")
            input.bodyText = value;
        if (key === "headings")
        ...
        if (key === "ctas")
            input.ctas = parseJsonFlag(value, []);
        if (key === "pageType" || key === "page-type")
            input.pageType = value;
        if (key === "primaryKeyword" || key === "primary-keyword")
            input.primaryKeyword = value;
        if (key === "secondaryKeywords" || key === "secondary-keywords")
            input.secondaryKeywords = parseJsonFlag(value, []);
...
function extractFromHtml(html) {
    ...
    const headings = [...html.matchAll(/<(h[1-6])\b[^>]*>([\s\S]*?)<\/\1>/giu)].map((match) => ({ level: match[1].toLowerCase(), text: stripTags(match[2]).trim() }));
    const images = [...html.matchAll(/<img\b[^>]*>/giu)].map((match) => ({ src: attr(match[0], "src") ?? "", alt: attr(match[0], "alt") }));
    const links = [...html.matchAll(/<a\b[^>]*href\s*=\s*["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/giu)].map((match) => ({ href: match[1], text: stripTags(match[2]).trim(), type: classifyLink(match[1]) }));
    const ctas = links.filter((link) => /\b(book|buy|get|start|try|contact|demo|pricing|quote|signup|sign up)\b/iu.test(link.text ?? "")).map((link) => ({ text: link.text ?? "", href: link.href, position: "unknown" }));
...
function getMissingInputs(input) {
    ...
    if (!input.h1 && !input.headings?.some((heading) => heading.level === "h1"))
        missing.push("h1");
    if (!input.bodyText)
        missing.push("bodyText");
...
//# sourceMappingURL=on-page-audit.js.map