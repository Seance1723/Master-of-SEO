...
function inferNaturalCommand(input) {
    if (/\b(keyword|keywords|intent)\b/u.test(input))
        return "keyword-research";
    if (/\b(competitor|serp)\b/u.test(input))
        return "competitor-analysis";
    if (/\b(plan|strategy|roadmap)\b/u.test(input))
        return "seo-plan";
    if (/\b(report|measurement|governance)\b/u.test(input))
        return "report";
    if (/\b(qa|release)\b/u.test(input))
        return "seo-qa";
    if (/\b(technical|html|audit|website|site|wrong)\b/u.test(input))
