...
export function checkUrlStructure(input) {
    const url = input.url;
    if (!url)
        return { issues: [], passedChecks: [] };
    const issues = [];
    const add = (id, title, problem, evidence, priority = "P2") => {
        issues.push({
            id,
            category: "url structure",
            title,
    ...
    if (/[A-Z]/u.test(url))
        add("url-uppercase", "Uppercase URL characters", "URL contains uppercase characters.", [url], "P3");
    if (/_/u.test(url))
        add("url-underscores", "URL uses underscores", "URL contains underscores instead of hyphens.", [url], "P3");
    if (/[?&](phpsessid|sessionid|sid|jsessionid)=/iu.test(url))
        add("url-session-id", "Session ID in URL", "URL contains a session identifier.", [url], "P1");
    const query = url.split("?")[1] ?? "";
    if (query && query.split("&").filter(Boolean).length > 4)
        add("url-excessive-query-params", "Excessive query parameters", "URL has many query parameters.", [url], "P2");
    if (url.length > 115)
        add("url-too-long", "Very long URL", "URL is longer than 115 characters.", [`${url.length} characters`], "P3");
    return { issues, passedChecks: issues.length ? [] : ["URL structure checks passed for provided URL."] };
}
//# sourceMappingURL=url-structure.js.map