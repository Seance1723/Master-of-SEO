export function pageTypeForCluster(cluster) {
    if (cluster.intent === "product_led")
        return "product";
    if (cluster.intent === "commercial")
        return cluster.recommendedPageType === "comparison" ? "comparison" : "landing";
    if (cluster.intent === "transactional")
        return cluster.recommendedPageType === "product" ? "product" : "service";
    if (cluster.intent === "informational")
        return "blog";
    if (cluster.intent === "local")
        return "local";
    if (cluster.intent === "pricing")
        return "pricing";
    if (cluster.intent === "support")
        return "documentation";
    return cluster.recommendedPageType ?? "unknown";
}
export function makeContentItem(cluster, kind, index, pageType, priority) {
...
//# sourceMappingURL=content-brief.js.map