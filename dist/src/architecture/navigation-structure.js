...
export function checkNavigation(input, graph) {
    const issues = [];
    ...
    if (topLevel.length > 12)
        issues.push(issue("navigation-too-many-top-level", "Too many top-level navigation items", "P2", `${topLevel.length} top-level items`));
    for (const item of input.navigation ?? []) {
        if (/^(click here|more|link)$/iu.test(item.label.trim()))
            issues.push(issue("navigation-vague-label", "Vague navigation label", "P3", item.label));
        if (!known.has(normalizeUrl(item.url)))
            issues.push(issue("navigation-url-not-found", "Navigation URL not found in pages or sitemap", "P3", item.url));
    }
    for (const page of input.pages ?? []) {
        const incoming = graph.incoming.get(normalizeUrl(page.url)) ?? [];
        if ((page.importance === "critical" || page.importance === "high") && !navUrls.has(normalizeUrl(page.url)) && incoming.length < 3)
            issues.push(issue("important-page-missing-navigation", "Important page missing from navigation with weak links", "P2", page.url));
    }
    return issues;
}
...
//# sourceMappingURL=navigation-structure.js.map