...
export function checkBreadcrumbs(input) {
    const issues = [];
    ...
    for (const page of input.pages ?? []) {
        ...
        if (hierarchical && !trail)
            issues.push(issue("breadcrumbs-missing", "Missing breadcrumbs on hierarchical page", "P2", page.url));
        if (trail) {
            if (!trail.items.some((item) => item.url === "/" || item.position === 1 || /home/i.test(item.label)))
                issues.push(issue("breadcrumbs-missing-home", "Breadcrumb path missing homepage/root", "P3", page.url));
            for (const item of trail.items.slice(0, -1))
                if (!item.url)
                    issues.push(issue("breadcrumb-item-missing-url", "Breadcrumb item missing URL", "P3", item.label));
            const pathParts = normalizeUrl(page.url).split("/").filter(Boolean);
            if (pathParts.length > 1 && trail.items.length > 1 && !normalizeUrl(page.url).includes(normalizeUrl(trail.items[trail.items.length - 1].url ?? page.url)))
                issues.push(issue("breadcrumb-conflicts-hierarchy", "Breadcrumb may conflict with URL hierarchy", "P3", page.url));
        }
    }
    return issues;
}
