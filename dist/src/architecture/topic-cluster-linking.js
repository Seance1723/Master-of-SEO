...
export function checkTopicClusterLinking(input) {
    const issues = [];
    ...
    for (const cluster of input.topicClusters ?? []) {
        if (!cluster.pillarUrl) {
            issues.push(issue("topic-cluster-missing-pillar", "Topic cluster missing pillar page", "P1", cluster.clusterName));
            continue;
        }
        ...
        for (const support of cluster.supportingUrls ?? []) {
            ...
            if (!links.some((link) => link.from === normalizedSupport && link.to === pillar))
                issues.push(issue("support-not-linking-pillar", "Supporting page does not link to pillar", "P2", `${support} -> ${cluster.pillarUrl}`));
            if (!links.some((link) => link.from === pillar && link.to === normalizedSupport))
                issues.push(issue("pillar-not-linking-support", "Pillar does not link to supporting page", "P2", `${cluster.pillarUrl} -> ${support}`));
        }
        if ((cluster.supportingUrls ?? []).length > 1) {
            const supportSet = (cluster.supportingUrls ?? []).map(normalizeUrl);
            if (!links.some((link) => supportSet.includes(link.from) && supportSet.includes(link.to) && link.from !== link.to))
                issues.push(issue("supporting-pages-isolated", "Supporting pages are isolated from each other", "P3", cluster.clusterName));
        }
    }
    return issues;
}
...
//# sourceMappingURL=topic-cluster-linking.js.map