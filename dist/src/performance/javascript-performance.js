export const javascriptPerformanceRules = [
    {
...
export function checkJavaScriptPerformance(input) {
    const issues = [];
    const scripts = (input.assets ?? []).filter((asset) => asset.type === "script" || asset.type === "third_party");
    for (const asset of scripts) {
        if (asset.loading === "blocking" && asset.position === "above_fold")
            issues.push(issue("js-blocking-head", "Blocking script near page start", "P1", asset, "Script is marked blocking above the fold."));
        else if (asset.loading === "blocking")
            issues.push(issue("js-blocking", "Blocking script", "P2", asset, "Script is marked blocking."));
        if ((asset.sizeKb ?? 0) > 1000)
            issues.push(issue("js-very-large", "Very large JavaScript asset", "P1", asset, `Script is ${asset.sizeKb}KB.`));
        else if ((asset.sizeKb ?? 0) > 300)
            issues.push(issue("js-large", "Large JavaScript asset", "P2", asset, `Script is ${asset.sizeKb}KB.`));
        if (asset.type === "third_party")
            issues.push(issue("third-party-script-review", "Third-party script needs review", "P3", asset, "Third-party script is present."));
    }
    const htmlScriptCount = (input.html?.match(/<script\b/giu) ?? []).length;
    if (htmlScriptCount > 20)
        issues.push(htmlIssue("html-many-scripts", "Many script tags", "P2", `${htmlScriptCount} script tags found.`));
...
//# sourceMappingURL=javascript-performance.js.map