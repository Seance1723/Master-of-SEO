...
test("input /seo-master memory reads memory", async () => {
  ...
  assert.match(result.message, /Current group: Site Architecture & Internal Linking/u);
});

test("input /seo-master next-group returns Group 7", async () => {
  const result = await runSeoMaster("/seo-master next-group");
  assert.equal(result.type, "next-group");
  assert.match(result.message, /Site Architecture & Internal Linking \(group-07\)/u);
});
