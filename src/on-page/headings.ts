...
export const headingRules: OnPageRule[] = [{
...
export function checkHeadings(input: OnPageAuditInput): { issues: OnPageAuditIssue[]; passedChecks: string[] } {
  const headings = input.headings ?? [];
  const h1s = headings.filter((heading) => heading.level === "h1");
  const issues: OnPageAuditIssue[] = [];
  if (!input.h1 && h1s.length === 0) issues.push(issue("h1-missing", "Missing H1", "P1", "No H1 was provided or extracted.", "Add one clear H1 describing the page."));
  if (h1s.length > 1) issues.push(issue("h1-multiple", "Multiple H1s found", "P2", `${h1s.length} H1 headings found.`, "Use one primary H1 and structure subtopics with H2/H3."));
  if (h1s.some((heading) => !heading.text.trim()) || input.h1?.trim() === "") issues.push(issue("h1-empty", "Empty H1", "P1", "H1 is empty.", "Write a descriptive H1."));
  const h1Text = input.h1 ?? h1s[0]?.text ?? "";
  if (/^(welcome|home)$/iu.test(h1Text.trim())) issues.push(issue("h1-vague", "H1 is too vague", "P2", h1Text, "Make the H1 specific to the page topic."));
  if (isContentHeavy(input) && !headings.some((heading) => heading.level === "h2")) issues.push(issue("h2-missing-content-heavy", "Missing H2s on content-heavy page", "P2", "No H2 headings found.", "Use H2s to organize important sections."));
  for (let index = 1; index < headings.length; index++) {
    const prev = Number(headings[index - 1].level.slice(1));
    const current = Number(headings[index].level.slice(1));
    if (current - prev > 1) issues.push(issue("heading-skipped-hierarchy", "Skipped heading hierarchy", "P3", `${headings[index - 1].level} to ${headings[index].level}`, "Avoid large jumps in heading hierarchy."));
  }
  ...
  if (repeated) issues.push(issue("heading-repeated-text", "Repeated heading text", "P3", repeated, "Make repeated headings more specific."));
  const long = headings.find((heading) => heading.text.length > 90);
  if (long) issues.push(issue("heading-too-long", "Very long heading", "P3", long.text, "Shorten headings for scanability."));
  return { issues, passedChecks: headings.length && !issues.length ? ["Heading checks passed."] : [] };
}

function isContentHeavy(input: OnPageAuditInput): boolean {
