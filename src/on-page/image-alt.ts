...
export const imageAltRules: OnPageRule[] = [{
  id: "image-alt-core",
  category: "image-alt",
...
export function checkImageAlt(input: OnPageAuditInput): { issues: OnPageAuditIssue[]; passedChecks: string[] } {
  const images = input.images ?? [];
  const issues: OnPageAuditIssue[] = [];
  for (const image of images) {
    if (image.alt === undefined) issues.push(issue("image-alt-missing", "Image missing alt text", image.position === "above_fold" ? "P2" : "P3", image.src, "Add meaningful alt text or an empty alt for decorative images."));
    else if (isStuffed(image.alt)) issues.push(issue("image-alt-keyword-stuffed", "Image alt may be keyword-stuffed", "P2", image.alt, "Rewrite alt text naturally."));
  }
  ...
  if (duplicate) issues.push(issue("image-alt-duplicate-many", "Duplicate alt text across many images", "P3", duplicate, "Make alt text specific to each meaningful image."));
