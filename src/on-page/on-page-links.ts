...
export const onPageLinkRules: OnPageRule[] = [{
...
export function checkOnPageLinks(input: OnPageAuditInput): { issues: OnPageAuditIssue[]; passedChecks: string[] } {
  const links = input.links ?? [];
  const issues: OnPageAuditIssue[] = [];
  if (input.bodyText && !links.some((link) => link.type === "internal" || (link.type === "unknown" && link.href.startsWith("/")))) issues.push(issue("internal-links-missing", "No internal links found", "P2", "No internal links were provided or extracted.", "Add relevant body links to related pages."));
  const genericCount = links.filter((link) => /^(click here|read more|learn more|here)$/iu.test(link.text?.trim() ?? "")).length;
  if (genericCount >= Math.max(1, links.length)) issues.push(issue("generic-anchor-text", "Generic anchor text overused", "P3", `${genericCount} generic anchors found.`, "Use descriptive anchors that explain destination value."));
  for (const link of links) {
    if (link.type === "external" && !link.text?.trim()) issues.push(issue("external-link-missing-anchor", "External link missing anchor text", "P3", link.href, "Add descriptive anchor text."));
    if (/^javascript:/iu.test(link.href)) issues.push(issue("javascript-link", "JavaScript link found", "P2", link.href, "Use a crawlable href for important links."));
  }
