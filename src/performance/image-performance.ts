...
export const imagePerformanceRules: PerformanceRule[] = [
  {
...
export function checkImagePerformance(input: PerformanceAuditInput): { issues: PerformanceAuditIssue[]; passedChecks: string[] } {
  const issues: PerformanceAuditIssue[] = [];
  const imageAssets = (input.assets ?? []).filter((asset) => asset.type === "image");

  for (const asset of imageAssets) {
    if ((asset.sizeKb ?? 0) > 1000) issues.push(issue("image-very-large", "Very large image asset", "P1", asset, `Image is ${asset.sizeKb}KB.`));
    else if ((asset.sizeKb ?? 0) > 300) issues.push(issue("image-large", "Large image asset", "P2", asset, `Image is ${asset.sizeKb}KB.`));
    if (!asset.width || !asset.height) issues.push(issue("image-missing-dimensions", "Image missing width/height", "P2", asset, "Image asset lacks width or height."));
    if (asset.position === "above_fold" && asset.loading === "lazy") issues.push(issue("hero-image-lazy-loaded", "Above-fold hero image is lazy-loaded", "P1", asset, "Above-fold image uses lazy loading."));
    if (asset.format && !["webp", "avif", "svg"].includes(asset.format.toLowerCase())) issues.push(issue("image-non-modern-format", "Non-modern image format", "P2", asset, `Image format is ${asset.format}.`));
  }

  return { issues, passedChecks: imageAssets.length && !issues.length ? ["Image asset checks passed."] : [] };
}

function issue(id: string, title: string, priority: "P1" | "P2", asset: PerformanceAsset, evidence: string): PerformanceAuditIssue {
  return {
    id,
    category: "image-performance",
    title,
