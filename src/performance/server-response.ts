...
export const serverResponseRules: PerformanceRule[] = [
  {
...
export function checkServerResponse(input: PerformanceAuditInput): { issues: PerformanceAuditIssue[]; passedChecks: string[] } {
  const ttfb = input.metrics?.ttfb;
  if (ttfb === undefined) return { issues: [], passedChecks: [] };
  if (ttfb > 1800) return { issues: [issue("ttfb-slow", "TTFB is slow", "P1", `TTFB is ${ttfb}ms.`)], passedChecks: [] };
  if (ttfb > 800) return { issues: [issue("ttfb-needs-improvement", "TTFB needs improvement", "P2", `TTFB is ${ttfb}ms.`)], passedChecks: [] };
  return { issues: [], passedChecks: ["TTFB is within the good threshold."] };
}

function issue(id: string, title: string, priority: "P1" | "P2", evidence: string): PerformanceAuditIssue {
  return {
    id,
    category: "server-response",
    title,
