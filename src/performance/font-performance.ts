...
export const fontPerformanceRules: PerformanceRule[] = [
  {
...
export function checkFontPerformance(input: PerformanceAuditInput): { issues: PerformanceAuditIssue[]; passedChecks: string[] } {
  const issues: PerformanceAuditIssue[] = [];
  const fonts = (input.assets ?? []).filter((asset) => asset.type === "font");
  if (fonts.length > 4) issues.push(issue("font-too-many-files", "Many font files", "P2", `${fonts.length} font files provided.`));

  const fontWeights = new Set(fonts.map((font) => font.url.match(/(?:-|_)([1-9]00)(?:\.|-|_)/u)?.[1]).filter(Boolean));
  if (fontWeights.size > 4) issues.push(issue("font-too-many-weights", "Many font weights", "P3", `${fontWeights.size} font weights inferred from asset names.`));
  ...
  if (hasFontAsset && input.html && !hasPreload) issues.push(issue("font-missing-preload", "Critical font preload not found", "P3", "Font usage detected without a font preload hint."));

  return { issues, passedChecks: fonts.length && !issues.length ? ["Font checks passed."] : [] };
}

function issue(id: string, title: string, priority: "P2" | "P3", evidence: string): PerformanceAuditIssue {
  return {
    id,
    category: "font-performance",
    title,
