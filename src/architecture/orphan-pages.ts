...
export function detectOrphanPages(input: ArchitectureAuditInput, graph: LinkGraph): { orphanPages: string[]; issues: ArchitectureIssue[] } {
  ...
  for (const sitemapUrl of input.sitemapUrls ?? []) if (!known.has(normalizeUrl(sitemapUrl))) issues.push(issue("sitemap-only-url", "Sitemap-only URL found", "P2", sitemapUrl));
