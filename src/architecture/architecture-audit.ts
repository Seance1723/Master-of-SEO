import { checkAnchorText } from "./anchor-text.ts";
import { buildArchitectureRecommendations } from "./architecture-recommendations.ts";
import { checkBreadcrumbs } from "./breadcrumbs.ts";
import { calculateDepthIssues } from "./crawl-depth.ts";
import { buildLinkGraph } from "./internal-linking.ts";
import { recommendLinkEquity } from "./link-equity.ts";
import { checkNavigation } from "./navigation-structure.ts";
import { detectOrphanPages } from "./orphan-pages.ts";
import { checkTopicClusterLinking } from "./topic-cluster-linking.ts";
import { checkUrlHierarchy } from "./url-hierarchy.ts";
import { normalizeUrl } from "./url-utils.ts";
import type { ArchitectureAuditInput, ArchitectureAuditOutput, ArchitectureIssue } from "../types/architecture.ts";

export function runArchitectureAudit(input: ArchitectureAuditInput): ArchitectureAuditOutput {
  const normalized: ArchitectureAuditInput = { ...input, mode: input.mode ?? "audit" };
  const missingInputs = getMissingInputs(normalized);
  if (!hasAnyInput(normalized)) return needsInput(missingInputs);
...
export function parseArchitectureAuditInputFromText(rawInput: string): ArchitectureAuditInput {
  const input: ArchitectureAuditInput = { mode: "audit" };
  ...
  for (const match of args.matchAll(flagPattern)) {
    ...
    if (key === "navigation") input.navigation = parseJsonFlag(value, []);
    if (key === "breadcrumbs") input.breadcrumbs = parseJsonFlag(value, []);
    if (key === "sitemapUrls" || key === "sitemap-urls") input.sitemapUrls = parseJsonFlag(value, []);
    if (key === "topicClusters" || key === "topic-clusters") input.topicClusters = parseJsonFlag(value, []);
    if (key === "mode" && ["audit", "planning", "linking", "architecture"].includes(value)) input.mode = value as ArchitectureAuditInput["mode"];
  }
  return input;
}

function checkInternalLinking(input: ArchitectureAuditInput, graph: ReturnType<typeof buildLinkGraph>): ArchitectureIssue[] {
  const issues: ArchitectureIssue[] = [];
  for (const page of input.pages ?? []) {
    const url = normalizeUrl(page.url);
    const incoming = graph.incoming.get(url) ?? [];
    if (["critical", "high"].includes(page.importance ?? "") && incoming.length < 3) issues.push(issue("important-page-too-few-internal-links", "Important page has too few incoming links", "P2", `${page.url} incoming=${incoming.length}`));
    if (["bofu"].includes(page.funnelStage ?? "") || ["service", "product", "pricing"].includes(page.pageType ?? "")) {
      if (!incoming.some((link) => link.linkType === "contextual")) issues.push(issue("commercial-page-no-contextual-links", "Commercial/BOFU page has no contextual incoming links", page.importance === "critical" ? "P1" : "P2", page.url));
    }
    if (page.intent === "informational") {
      const outgoing = graph.outgoing.get(url) ?? [];
      if (!outgoing.some((link) => ["service", "product", "pricing", "landing"].some((part) => link.to.includes(part)))) issues.push(issue("informational-page-no-commercial-links", "Informational page has no commercial internal links", "P2", page.url));
    }
  }
  for (const link of input.links ?? []) if (link.isFollowed === false) issues.push(issue("nofollow-internal-link", "Nofollow internal link found", "P2", `${link.from} -> ${link.to}`));
  return issues;
}
...
function needsInput(missingInputs: string[]): ArchitectureAuditOutput {
...
function hasAnyInput(input: ArchitectureAuditInput): boolean {
...
function getMissingInputs(input: ArchitectureAuditInput): string[] {
  const missing: string[] = [];
  if (!input.pages?.length) missing.push("pages");
  if (!input.links?.length) missing.push("links");
  if (!input.navigation?.length) missing.push("navigation");
  if (!input.breadcrumbs?.length) missing.push("breadcrumbs");
  if (!input.sitemapUrls?.length) missing.push("sitemapUrls");
  if (!input.topicClusters?.length) missing.push("topicClusters");
  return missing;
}
...
function score(issues: ArchitectureIssue[]): number {
