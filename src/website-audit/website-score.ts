...
export function scoreFromIssues(issues: WebsiteAuditIssue[]): number {
  return Math.max(0, 100 - issues.reduce((sum, issue) => sum + ({ P0: 20, P1: 8, P2: 3, P3: 1 }[issue.priority]), 0));
}

export function calculateCategoryScore(issues: WebsiteAuditIssue[], sourceSkill: string): number {
  return scoreFromIssues(issues.filter((issue) => issue.sourceSkill === sourceSkill || issue.sourceSkills?.includes(sourceSkill)));
}

export function calculateWebsiteScore(categoryScores: WebsiteAuditOutput["categoryScores"]): number {
  const applicable = Object.entries(categoryScores).filter((entry): entry is [keyof typeof auditCategoryWeights, number] => typeof entry[1] === "number");
...
export function gradeFromScore(score: number): WebsiteAuditGrade {
