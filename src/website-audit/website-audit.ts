...
import { runInternationalSEOAudit } from "../local-international/international-seo-audit.ts";
...
import type { WebsiteAuditInput, WebsiteAuditIssue, WebsiteAuditOutput, WebsiteType, BusinessGoal, WebsiteAuditPage } from "../types/website-audit.ts";
import { generateRoadmap } from "./audit-roadmap.ts";
import { getQuickWins, getStrategicOpportunities } from "./audit-report-generator.ts";
import { deduplicateIssues } from "./issue-deduplication.ts";
import { runTemplateAudit } from "./template-audit.ts";
import { calculateCategoryScore, calculateWebsiteScore, gradeFromScore } from "./website-score.ts";

export function runWebsiteAudit(input: WebsiteAuditInput): WebsiteAuditOutput {
  if (!hasInput(input)) return needsInput();
  if (input.url && !hasInput({ ...input, url: undefined })) return needsInput(["website", "pages", "manual audit data"]);
  const websiteType = detectWebsiteType(input);
  const businessGoal = detectBusinessGoal(input, websiteType);
  const rawIssues: WebsiteAuditIssue[] = [];
  ...
  runModule("schema-entity-seo", rawIssues, categoryFindings, () => input.schema ? runSchemaAudit({ mode: "audit", html: input.pages?.[0]?.html, jsonLd: input.schema.jsonLd, page: input.pages?.[0] as never, organization: input.schema.organization, author: input.schema.author, product: input.schema.product, service: input.schema.service, softwareApplication: input.schema.softwareApplication, localBusiness: input.schema.localBusiness, video: input.schema.video, jobPosting: input.schema.jobPosting }) : undefined);
  runModule("media-seo", rawIssues, categoryFindings, () => input.media ? runMediaAudit({ mode: "audit", html: input.pages?.[0]?.html, page: input.pages?.[0] as never, images: input.media.images, videos: input.media.videos, openGraph: input.media.openGraph, assets: input.media.assets }) : undefined);
  if (websiteType === "ecommerce" || input.ecommerce) runModule("ecommerce-seo", rawIssues, categoryFindings, () => input.ecommerce ? runEcommerceAudit({ mode: "audit", page: input.pages?.[0] as never, categories: input.ecommerce.categories, products: input.ecommerce.products, filters: input.ecommerce.filters, pagination: input.ecommerce.pagination, policies: input.ecommerce.policies, merchantFeed: input.ecommerce.merchantFeed, internalLinks: input.ecommerce.internalLinks }) : undefined);
  if (input.localInternational) {
    runModule("local-international-seo", rawIssues, categoryFindings, () => input.localInternational?.business || input.localInternational?.locations ? runLocalSEOAudit({ mode: "audit", business: input.localInternational.business, locations: input.localInternational.locations, googleBusinessProfile: input.localInternational.googleBusinessProfile, citations: input.localInternational.citations, reviews: input.localInternational.reviews, pages: input.pages as never }) : undefined);
    runModule("local-international-seo", rawIssues, categoryFindings, () => input.localInternational?.site || input.localInternational?.hreflangSets || input.localInternational?.localizedContent ? runInternationalSEOAudit({ mode: "audit", site: input.localInternational.site, pages: input.pages as never, hreflangSets: input.localInternational.hreflangSets, localizedContent: input.localInternational.localizedContent }) : undefined);
  }
  runModule("ai-search-discover-seo", rawIssues, categoryFindings, () => input.aiDiscover ? runAISearchAudit({ mode: "audit", page: input.pages?.[0] as never, html: input.pages?.[0]?.html, content: input.aiDiscover.content, entities: input.aiDiscover.entities as never, queries: input.aiDiscover.queries as never }) : undefined);
  runModule("ai-search-discover-seo", rawIssues, categoryFindings, () => input.aiDiscover?.publisher || input.aiDiscover?.contentSignals ? runDiscoverSEOAudit({ mode: "discover", page: input.pages?.[0] as never, publisher: input.aiDiscover.publisher as never, contentSignals: input.aiDiscover.contentSignals }) : undefined);
  if (input.trustSecurityAccessibility) {
    runModule("trust-security-accessibility", rawIssues, categoryFindings, () => runTrustAudit({ mode: "audit", page: input.pages?.[0] as never, organization: input.trustSecurityAccessibility?.organization, authors: input.trustSecurityAccessibility?.authors as never, reviewers: input.trustSecurityAccessibility?.reviewers as never, trustPages: input.trustSecurityAccessibility?.trustPages, testimonials: input.trustSecurityAccessibility?.testimonials as never, caseStudies: input.trustSecurityAccessibility?.caseStudies as never }));
    runModule("trust-security-accessibility", rawIssues, categoryFindings, () => runSecurityAudit({ mode: "audit", page: input.pages?.[0] as never, headers: input.trustSecurityAccessibility?.headers, resources: input.trustSecurityAccessibility?.resources as never, forms: input.trustSecurityAccessibility?.forms as never, securitySignals: input.trustSecurityAccessibility?.securitySignals }));
    runModule("trust-security-accessibility", rawIssues, categoryFindings, () => runAccessibilityAudit({ mode: "audit", page: input.pages?.[0] as never, headings: input.trustSecurityAccessibility?.headings as never, images: input.trustSecurityAccessibility?.images as never, links: input.trustSecurityAccessibility?.links as never, buttons: input.trustSecurityAccessibility?.buttons as never, forms: input.trustSecurityAccessibility?.forms as never, accessibilitySignals: input.trustSecurityAccessibility?.accessibilitySignals }));
  }
...
export function parseWebsiteAuditInputFromText(rawInput: string): WebsiteAuditInput {
  const input: WebsiteAuditInput = { mode: rawInput.includes("page-audit") ? "page" : "website" };
  const args = rawInput.replace(/^\/seo-master\s+(?:audit-website|website-audit|full-audit|page-audit)\s*/u, "").trim();
  if (/^https?:\/\//iu.test(args)) input.url = args;
  for (const match of args.matchAll(/--([a-zA-Z][\w-]*)(?:\s+(?:"([^"]*)"|'([^']*)'|(\S+)))?/gu)) {
    const key = match[1];
    const value = match[2] ?? match[3] ?? match[4] ?? "";
    if (key === "url") input.url = value;
    if (key === "website") input.website = parseJsonFlag(value, undefined);
    if (key === "pages") input.pages = parseJsonFlag(value, []);
    if (key === "technical") input.technical = parseJsonFlag(value, undefined);
    if (key === "performance") input.performance = parseJsonFlag(value, undefined);
    if (key === "onPage" || key === "on-page") input.onPage = parseJsonFlag(value, undefined);
    if (key === "keywords") input.keywords = parseJsonFlag(value, undefined);
    if (key === "content") input.content = parseJsonFlag(value, undefined);
    if (key === "architecture") input.architecture = parseJsonFlag(value, undefined);
    if (key === "schema") input.schema = parseJsonFlag(value, undefined);
    if (key === "media") input.media = parseJsonFlag(value, undefined);
    if (key === "ecommerce") input.ecommerce = parseJsonFlag(value, undefined);
    if (key === "localInternational" || key === "local-international") input.localInternational = parseJsonFlag(value, undefined);
    if (key === "aiDiscover" || key === "ai-discover") input.aiDiscover = parseJsonFlag(value, undefined);
    if (key === "trustSecurityAccessibility" || key === "trust-security-accessibility") input.trustSecurityAccessibility = parseJsonFlag(value, undefined);
    if (key === "cmsFramework" || key === "cms-framework") input.cmsFramework = parseJsonFlag(value, undefined);
  }
  return input;
}

function runModule(sourceSkill: string, issues: WebsiteAuditIssue[], findings: string[], run: () => { score?: number; status?: string; issues?: unknown[]; summary?: string } | undefined): void {
  const result = run();
  if (!result || result.status === "needs_input") return;
  findings.push(`${sourceSkill}: ${result.summary ?? "completed"}`);
  for (const item of result.issues ?? []) issues.push(convertIssue(item as Record<string, unknown>, sourceSkill));
}

function convertIssue(item: Record<string, unknown>, sourceSkill: string): WebsiteAuditIssue {
  return {
    id: String(item.id ?? item.title ?? "issue"),
    category: String(item.category ?? sourceSkill),
    sourceSkill,
    title: String(item.title ?? "Audit issue"),
    priority: (item.priority as WebsiteAuditIssue["priority"]) ?? "P2",
...
function addPageIssues(pages: WebsiteAuditPage[], issues: WebsiteAuditIssue[]): void {
  for (const page of pages) {
    if (!page.title) issues.push(pageIssue("page-missing-title", "Missing title on provided page", "P1", page));
    if (!page.metaDescription) issues.push(pageIssue("page-missing-meta-description", "Missing meta description on provided page", "P2", page));
    if (!page.h1) issues.push(pageIssue("page-missing-h1", "Missing H1 on provided page", "P1", page));
    if (page.isIndexable === false && ["homepage", "service", "product", "category", "landing", "pricing"].includes(page.pageType ?? "")) issues.push(pageIssue("important-page-noindex", "Important page is noindex", "P1", page));
  }
}

function pageIssue(id: string, title: string, priority: WebsiteAuditIssue["priority"], page: WebsiteAuditPage): WebsiteAuditIssue {
...
function buildPageFindings(input: WebsiteAuditInput): string[] {
  return (input.pages ?? []).map((page) => `${page.url}: ${page.pageType ?? "unknown"}${page.importance ? ` (${page.importance})` : ""}`);
}

function detectWebsiteType(input: WebsiteAuditInput): WebsiteType {
  if (input.website?.websiteType) return input.website.websiteType;
  if (input.ecommerce?.products?.length || input.ecommerce?.categories?.length || input.pages?.some((page) => ["product", "category"].includes(page.pageType ?? ""))) return "ecommerce";
  if (input.localInternational?.business || input.localInternational?.locations?.length || input.pages?.some((page) => page.pageType === "local")) return "local_business";
  if (input.pages?.some((page) => ["blog", "article"].includes(page.pageType ?? ""))) return "blog_news";
  if (input.pages?.some((page) => ["pricing", "documentation"].includes(page.pageType ?? "")) || input.cmsFramework?.framework === "nextjs") return "saas";
  if (input.pages?.some((page) => ["service", "contact", "about"].includes(page.pageType ?? ""))) return "corporate";
  return "unknown";
}

function detectBusinessGoal(input: WebsiteAuditInput, type: WebsiteType): BusinessGoal {
  if (input.website?.businessGoal) return input.website.businessGoal;
  if (type === "ecommerce") return "sales";
  if (type === "local_business") return "local_visits";
  if (type === "blog_news") return "content_traffic";
  if (type === "saas") return input.pages?.some((page) => page.pageType === "pricing") ? "demo_booking" : "signup";
  if (type === "corporate") return "lead_generation";
  return "unknown";
}

function hasInput(input: WebsiteAuditInput): boolean {
...
function getMissingInputs(input: WebsiteAuditInput): string[] {
  const missing: string[] = [];
  for (const key of ["website", "pages", "technical", "performance", "onPage", "keywords", "content", "architecture", "schema", "media", "ecommerce", "localInternational", "aiDiscover", "trustSecurityAccessibility", "cmsFramework"] as const) {
    if (!input[key]) missing.push(key);
  }
  return missing;
}

function needsInput(missingInputs = ["website", "pages", "technical", "performance", "onPage", "schema", "media", "manual audit data"]): WebsiteAuditOutput {
...
function applyGlobalIssuePenalty(score: number, issues: WebsiteAuditIssue[]): number {
