import type { WebsiteAuditIssue, WebsiteAuditPage } from "../types/website-audit.ts";

export function runTemplateAudit(pages: WebsiteAuditPage[] = []): { findings: string[]; issues: WebsiteAuditIssue[] } {
  const issues: WebsiteAuditIssue[] = [];
  const findings: string[] = [];
  const groups = new Map<string, WebsiteAuditPage[]>();
  for (const page of pages) groups.set(page.pageType ?? "unknown", [...(groups.get(page.pageType ?? "unknown") ?? []), page]);
...
function addRepeated(pages: WebsiteAuditPage[], id: string, title: string, template: string, issues: WebsiteAuditIssue[], priority: WebsiteAuditIssue["priority"] = "P2"): void {
