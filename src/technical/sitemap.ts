...
export const sitemapRules: TechnicalRule[] = [
  {
    id: "sitemap-core",
...
export function checkSitemap(input: TechnicalAuditInput): { issues: TechnicalAuditIssue[]; passedChecks: string[] } {
  const issues: TechnicalAuditIssue[] = [];
  const passedChecks: string[] = [];
  const xml = input.sitemapXml?.trim();
  if (!xml) return { issues, passedChecks };
...
function issue(id: string, title: string, problem: string, evidence: string[], priority: "P1" | "P2" | "P3" = "P1"): TechnicalAuditIssue {
  return {
    id,
    category: "xml sitemap",
    title,
