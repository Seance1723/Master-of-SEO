...
export const metaRobotsRules: TechnicalRule[] = [
  {
    id: "meta-robots-core",
...
export function checkMetaRobots(input: TechnicalAuditInput): MetaRobotsResult {
  ...
  if (!input.html) return { issues, passedChecks, hasNoindex: false };

  if (hasNoindex) passedChecks.push("Meta robots noindex directive detected for indexability review.");
...
function issue(id: string, title: string, problem: string, whyItMatters: string, howToFix: string, evidence: string[], priority: "P1" | "P2" | "P3" = "P2"): TechnicalAuditIssue {
  return {
    id,
    category: "meta robots",
    title,
