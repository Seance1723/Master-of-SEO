import type { KeywordBusinessContext, KeywordIntent } from "../types/keywords.ts";

const patterns: Array<[KeywordIntent, RegExp]> = [
...
export function detectKeywordIntent(keyword: string, business?: KeywordBusinessContext): KeywordIntent {
  ...
  if (/\bnear me|local\b/iu.test(value) || (business?.locations ?? []).some((location) => value.includes(` in ${location.toLowerCase()}`) || value.includes(location.toLowerCase()))) return "local";
  for (const [intent, pattern] of patterns) if (pattern.test(value)) return intent;
  if (hasBusinessName && !strongBofu) return "navigational";
  return "unknown";
}
